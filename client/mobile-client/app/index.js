...
const HomeScreen = ({ navigation }) => {
  ...
  const { deviceId, deviceName, deviceType, screenWidth, screenHeight } =
    deviceData;

  const handleRegister = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://${serverIp}/devices/${deviceId}`
      );
      if (response.data) {
        alert("Device already registered");
      } else {
        const response = await axios.post(
          `http://${serverIp}/devices/register`,
          deviceData
        );
      ...
      router.push(
        `/visualization?deviceId=${deviceId}&deviceName=${deviceName}`
      );
    } catch (error) {
